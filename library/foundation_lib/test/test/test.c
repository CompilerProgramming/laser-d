...
main_initialize(void) {
	...
	// Run the test suite unattended so assert/alert dialogs are suppressed.
	// Without this a failing assert calls system_message_box (a modal MessageBox
	// on Windows, alert on macOS) which blocks headless CI runners until timeout.
	application_t test_application = test_suite.application();
	test_application.flags |= APPLICATION_UNATTENDED;

	ret = foundation_initialize(test_suite.memory_system(), test_application, test_suite.config());
	if (ret == 0) {
