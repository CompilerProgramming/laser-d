...
| Structs and unions (`struct.dd`) | Restricted | Value storage, layout, fields, literals, methods, direct constructors, bit fields, named and anonymous unions, ordinary initialization, and context-free local structs are supported. Struct destructors, postblits, copy/move constructors, invariants, `alias this`, and hidden-context structs are rejected. Constructor delegation/disablement remains under review; modern operators are classified separately. |
...
| Remaining advanced struct behavior | Undecided | Constructor delegation and attribute-driven construction/copy disablement require separate review. Modern operator hooks have already been classified in the operator-overloading section. | None |
...
| Feature | Status | Decision | Tests |
| --- | --- | --- | --- |
| Fixed-size arrays | Implementation defect | Fixed-size arrays provide inline value storage in permitted static storage, stack variables, and containing aggregates. Context-typed array literals are intended to initialize fixed storage without allocation, but the current blanket dynamic-array-literal rejection also rejects this supported form. | `static_arrays_and_slices_accepted.d`; positive fixed-literal coverage needed |
