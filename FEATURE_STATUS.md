...
| Statements (`statement.dd`) | Restricted | Ordinary scalar control flow, direct array/slice and numeric-range iteration, integral/enum switches, transfers, value-type `with`, and `scope(exit)` are supported. String switches, exceptions, other scope guards, and synchronization are rejected; advanced implicit iteration remains under review. |
