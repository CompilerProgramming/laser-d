...
| Declaration conditions in `if` statements | Undecided | Review declaration-style `if` conditions, including inferred variables, scope, conversion tests, and any destructuring or pattern forms. Ordinary expression conditions are supported. | `spec/statement.dd` | None |
...
| Fixed-size array dimensions | Implementation defect | Integer-literal dimensions work. General integral compile-time dimension expressions are intended to work, but the current frontend misclassifies a manifest-identifier dimension as an associative-array type. | `static_arrays_and_slices_accepted.d`; positive symbolic-dimension coverage needed |
