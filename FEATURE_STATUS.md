...
| `const` and `immutable` | Restricted | D-source `const` is rejected. `immutable` retains its transitive D semantics and may be initialized at runtime for local values; manifest `enum` values represent compile-time constants. ImportC `const` is preserved. | `DESIGN.md`, `spec/const3.dd`, `spec/type.dd` | `const_rejected.d`, `immutable_accepted.d`, `immutable_static_storage_accepted.d`, ImportC fixtures |
...
| Statements (`statement.dd`) | Restricted | `scope(exit)` is supported. `try`, `catch`, `finally`, `throw`, `scope(success)`, `scope(failure)`, and `synchronized` are rejected. Ordinary control flow remains to be reviewed. |
...
| Type qualifiers (`const3.dd`) | Restricted | `immutable` is supported with normal D transitive semantics. `const` and `shared` are rejected. `inout`, conversions, and remaining initialization details require review. |
