...
| Explicit hash-table library | Supported | `laserd.hash` provides an insertion-ordered, machine-word key/value table over a borrowed caller-owned rpmalloc heap. Hashing and equality use explicit function pointers; numeric, C-string, and ASCII case-insensitive C-string policies are included. The library does not restore built-in associative-array syntax or hidden runtime management. | `DESIGN.md`, `spec-markdown/d-compatibility.md`, `library/README.md` | None | `library/test/hash.d` |
