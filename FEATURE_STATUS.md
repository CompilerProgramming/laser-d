...
| Function contracts (`contracts.dd`) | Rejected | `in` preconditions, `out` postconditions, and contract-style `do` bodies are rejected. Runtime assertion expressions are rejected separately. |
...
| Remaining primary expressions | Undecided | Non-array type properties require separate review. Function literals, ordinary template instances, `is` expressions, traits, import expressions, `typeid`, `new`, interpolation, and `this` are classified separately; array literals and string mixins retain their existing restrictions. | `dynamic_array_literal_rejected.d`, `associative_array_literal_rejected.d` |
