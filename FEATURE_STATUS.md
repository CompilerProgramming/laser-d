...
| ImportC (`importc.dd`) | Restricted | Already-preprocessed `.i` translation units support the reviewed standalone C11 and mixed Laser-D/C baseline. C source retains its own storage, qualifier, scalar, layout, entry-point, and linkage rules. Automatic preprocessing, headers, macros, atomics, vectors, assembly, reverse imports, builtins, and vendor extensions remain to be audited individually. |
