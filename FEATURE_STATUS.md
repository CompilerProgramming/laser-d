...
compiler. The normative Markdown specification under `spec-markdown/`, the tests
under `compiler/test/laser-d`, and the implementation must ultimately
agree with every decided entry. Files under `spec/` are non-normative
upstream review sources retained for traceability.
...
In the decided-feature table, **Laser-D specification** identifies normative
Markdown wording (and supporting project design where applicable).
**Upstream review source** records non-normative D material used to discover
and compare the feature. Keeping these columns separate preserves provenance
without importing upstream behavior into Laser-D.

## Decided cross-cutting rules

| Feature | Status | Decision | Laser-D specification | Upstream review source | Laser-D tests |
| --- | --- | --- | --- | --- | --- |
| D source compatibility | Supported | Laser-D introduces no new syntax; every Laser-D program is also a D program. | `DESIGN.md` | None | Needed |
...
| D runtime | Rejected | Programs may not depend on the D runtime. Only the C runtime is available. | `DESIGN.md` | Applicable upstream chapters require review | Coverage incomplete |
| C standard library bindings | Restricted | Laser-D provides declaration-only `core.stdc` modules for verified C ABI types and APIs. The initial subset is `config`, `stddef`, `stdint`, `stdarg`, `stdlib`, `string`, and basic `stdio`; symbols resolve from the platform C runtime without druntime. | `DESIGN.md`, `library/README.md` | `spec/betterc.dd` | `library/test/core_stdc.d` |
| Phobos-compatible compile-time traits | Restricted | `std.traits` provides a tested value-type and function-introspection subset. APIs requiring rejected types, qualifiers, runtime facilities, or unreviewed recursive adaptation are omitted and inventoried in the module. | `DESIGN.md`, `spec-markdown/d-compatibility.md`, `library/README.md` | None | `library/test/traits.d` |
| Explicit hash-table library | Supported | `laserd.hash` provides an insertion-ordered, machine-word key/value table over a borrowed caller-owned `laserd.memory.Arena`. Hashing and equality use explicit function pointers; numeric, C-string, and ASCII case-insensitive C-string policies are included. The table releases storage through the Arena without destroying it, and actual reclamation follows the selected Arena backend. The library does not restore built-in associative-array syntax or hidden runtime management. | `DESIGN.md`, `spec-markdown/d-compatibility.md`, `library/README.md` | None | `library/test/hash.d` |
| Explicit allocation library | Supported | `laserd.rpmalloc` exposes the reviewed rpmalloc general-allocation and caller-owned first-class heap APIs using their native function, type, and constant names without D aliases. An explicitly created `laserd.memory.Arena` provides zeroing raw and type-aligned allocation, overflow-checked typed arrays, preserving expansion, zero-extension on successful growth, and explicit backend destruction. Its rpmalloc, single fixed-region, and dynamically growing 8 KiB bump-region backends retain distinct ownership and thread-safety contracts. Allocation, ownership, heap thread confinement, and release remain explicit; these modules do not introduce language allocation or garbage collection. | `DESIGN.md`, `library/README.md` | None | `library/test/rpmalloc.d`, `library/test/memory.d` |
| Optional and result library | Supported | `laserd.result` provides `Optional` and `Result` value types for reporting failure as ordinary returned data, including a `void` value type for operations which produce no value on success. Overlapping value and error storage is sound because no member carries lifecycle behaviour. Every accessor is total, since the language cannot compel a caller to inspect a result; transformation uses explicit function pointers and context. The module is one available convention rather than a language-mandated result type. | `DESIGN.md`, `spec-markdown/errors.md`, `library/README.md` | None | `library/test/result.d` |
| Foundation utility and lifecycle library | Restricted | `laserd.foundation.base64`, `laserd.foundation.hash`, and `laserd.foundation.lifecycle` are the reviewed Foundation subset. The utility functions are allocation-free; lifecycle initialization uses rpmalloc and defines explicit process and per-thread ownership. Other Foundation modules remain unsupported unless separately reviewed. | `DESIGN.md`, `library/README.md` | None | `library/test/foundation.d` |
| Thread and synchronization library | Restricted | `laserd.thread` provides reviewed Foundation thread management plus nsync reader/writer mutexes and Mesa-style condition variables. Its acquire/release and start/join visibility contract applies at the foreign-library boundary and does not restore rejected D language-level threading constructs. | `DESIGN.md`, `library/README.md` | None | `library/test/foundation.d`, `library/test/thread_sync.d` |
| Process, pipe, and stream library | Restricted | `laserd.system` provides the reviewed Foundation process, unnamed-pipe, and opaque byte-stream subset with explicit handle ownership and process-reaping requirements. Native handles, typed serialization, filesystem streams, shell launch, and process-global exit remain outside the supported surface. | `DESIGN.md`, `library/README.md` | None | `library/test/process_pipe.d` |
...
| Function `nothrow` attribute | Restricted | Every function type is implicitly `nothrow`; spelling `nothrow` explicitly is rejected. | `DESIGN.md`, `spec-markdown/function.md`, `spec-markdown/attribute.md` | `spec/function.dd`, `spec/attribute.dd` | `implicit_function_attributes.d`, `explicit_function_attributes.d`, `explicit_function_type_attributes.d` |
| Function `@nogc` attribute | Restricted | Every function type is implicitly `@nogc`; spelling `@nogc` explicitly is rejected. | `DESIGN.md`, `spec-markdown/function.md`, `spec-markdown/attribute.md` | `spec/function.dd`, `spec/attribute.dd` | `implicit_function_attributes.d`, `explicit_function_attributes.d`, `explicit_function_type_attributes.d` |
| Function safety attributes | Restricted | Every function and function type is implicitly `@system`; `@safe`, `@trusted`, and explicit `@system` are rejected, and safety inference is disabled. | `DESIGN.md`, `spec-markdown/function.md`, `spec-markdown/attribute.md` | `spec/function.dd`, `spec/attribute.dd`, `spec/memory-safe-d.dd` | `implicit_function_attributes.d`, `explicit_safety_attributes_rejected.d` |
| Receiver lifetime inference | Restricted | Source-written `return` and `scope` lifetime annotations are rejected in both parameter and postfix member-function positions. For templated aggregate methods, the frontend retains inferred receiver relationships and rejects an interior pointer escaping a shorter-lived receiver. This is not general borrow checking and no equivalent guarantee is made for non-template methods. | `DESIGN.md`, `spec-markdown/function.md` | `spec/function.dd` | `parameter_annotations_rejected.d`, `receiver_lifetime_qualifiers_rejected.d`, `receiver_lifetime_inference.d` |
| Disable attribute | Rejected | Explicit `@disable` is rejected. Laser-D does not use an attribute to create unavailable declarations or nonconstructible/noncopyable value types. | `DESIGN.md`, `spec-markdown/attribute.md`, `spec-markdown/types.md#structs-and-unions` | `spec/attribute.dd`, `spec/struct.dd` | `disable_attribute_rejected.d` |
| Function purity | Rejected | D functions remain conservatively impure; explicit `pure` and purity inference are rejected. ImportC behavior is preserved. | `DESIGN.md`, `spec-markdown/function.md`, `spec-markdown/attribute.md` | `spec/function.dd`, `spec/attribute.dd` | `threading_and_purity_rejected.d`, `implicit_function_attributes.d` |
| Mutable static storage | Rejected | Mutable global, module, function-static, and aggregate-static storage is rejected. Manifest constants, deeply immutable static data, and ImportC globals remain supported. | `DESIGN.md`, `spec-markdown/types.md#declarations`, `spec-markdown/module.md` | `spec/declaration.dd`, `spec/module.dd` | `mutable_static_storage_rejected.d`, `mutable_local_static_storage_rejected.d`, `immutable_static_storage_accepted.d` |
| Type qualifiers | Restricted | `const` is supported as an aliasable read-only type view and `immutable` retains its transitive D semantics. Qualification composes canonically with pointers and fixed arrays. Postfix `const` function qualifiers, `inout`, and `shared` are rejected. Manifest `enum` values represent compile-time constants. ImportC `const` is preserved; other C qualifiers remain under explicit review. | `DESIGN.md`, `spec-markdown/types.md#type-qualifiers`, `spec-markdown/types.md#types` | `spec/const3.dd`, `spec/type.dd` | `const_type_qualifier_accepted.d`, `upstream_compilable_b20045.d`, `const_function_qualifier_rejected.d`, `inout_rejected.d`, `immutable_accepted.d`, `immutable_static_storage_accepted.d`, ImportC fixtures |
| Uniqueness-based immutable reference conversion | Undecided | Review whether and when uniqueness analysis may convert a mutable pointer or slice expression to immutable referenced storage. No ownership transfer is currently promised merely from a mutable reference value. | `spec-markdown/types.md#type-qualifiers` | `spec/const3.dd` | None |
| Native multithreading | Rejected | `shared`, `__gshared`, and `synchronized` are rejected in Laser-D source. C threading and atomics remain accessible through ImportC. | `DESIGN.md`, `spec-markdown/attribute.md`, `spec-markdown/statement.md` | `spec/attribute.dd`, `spec/statement.dd` | `threading_and_purity_rejected.d`, `gshared_rejected.d` |
| Exceptions and cleanup | Restricted | D exceptions and source `try`/`finally` are rejected. `scope(exit)` is the sole deterministic-cleanup syntax; `scope(success)` and `scope(failure)` are rejected. Internal try/finally nodes used to lower `scope(exit)` remain implementation details. | `DESIGN.md`, `spec-markdown/statement.md`, `spec-markdown/errors.md` | `spec/statement.dd`, `spec/errors.dd` | `scope_exit_accepted.d`, `scope_exit_runtime_semantics.d`, `exception_statements_rejected.d`, `scope_success_failure_rejected.d` |
| Ordinary control flow | Supported | Scalar conditionals and loops, direct fixed-array/slice and numeric-range iteration, integral and enum switches, labels, control transfers, and struct-value `with` statements are supported. Active `scope(exit)` guards run when control leaves their scopes. | `DESIGN.md`, `spec-markdown/statement.md` | `spec/statement.dd` | `structured_control_flow_accepted.d`, `control_transfers_accepted.d` |
| Compile-time sequence iteration | Supported | Both `static foreach` and ordinary `foreach` over compile-time tuples and sequences are expanded by the frontend without a runtime iteration protocol. | `DESIGN.md`, `spec-markdown/statement.md` | `spec/statement.dd` | `ctfe_accepted.d`, `compile_time_iteration_accepted.d` |
| Value-type ranges | Restricted | Struct ranges are supported through validated parameterless instance methods: `empty()` returns `bool`; `front()`/`back()` returns a supported non-`ref` value; and `popFront()`/`popBack()` returns `void`. | `DESIGN.md`, `spec-markdown/statement.md` | `spec/statement.dd` | `range_iteration_accepted.d`, `range_iteration_invalid_rejected.d` |
| Callback iteration protocols | Rejected | Struct `opApply`/`opApplyReverse` and delegate aggregates are rejected; ranges, explicit loops, or explicit calls must be used. | `DESIGN.md`, `spec-markdown/statement.md` | `spec/statement.dd` | `opapply_iteration_rejected.d`, `delegate_iteration_rejected.d` |
| String switches | Rejected | Switching on strings or character slices is rejected because D lowers it through the unavailable `object.__switch` runtime hook. | `DESIGN.md`, `spec-markdown/statement.md` | `spec/statement.dd` | `string_switch_rejected.d` |
| Effect-free expression statements | Undecided | Review which value-only expressions are diagnosed as having no effect and whether an explicit conversion to `void` suppresses that diagnostic in every context. | `spec-markdown/statement.md` | `spec/statement.dd` | None |
| Declaration conditions in `if` statements | Supported | `if (auto name = expression)` and `if (Type name = expression)` initialize once, test the resulting value, and expose the declared name only in the selected `then` branch. Declaration conditions in other statement kinds remain separate review items. | `spec-markdown/statement.md` | None | `if_declaration_conditions_accepted.d` |
| Declaration conditions outside `if` | Undecided | Review declaration conditions in `while`, `switch`, and `with` independently of the supported `if` form. Ordinary `for` initializers are not declaration conditions. | None | None | None |
| Extended `foreach` variable forms | Undecided | Review explicit iterator types, `enum`, type qualifiers, and `alias` iteration variables independently of supported inferred values, indices, and mutable `ref` iteration. | `spec-markdown/statement.md` | `spec/statement.dd` | Existing direct and compile-time iteration tests only |
| Non-struct `with` statements | Undecided | Review `with` over unions, enums, modules, namespaces, and other retained symbol scopes. The tested struct-value form remains supported. | `spec-markdown/statement.md` | `spec/statement.dd` | `control_transfers_accepted.d` covers structs only |
| Pragma statements | Undecided | Review statement-position pragmas individually. `pragma(msg)` is already rejected in declaration and statement positions by the compile-time I/O decision. | `spec-markdown/statement.md` | `spec/statement.dd` | `compile_time_statement_output_rejected.d` covers `pragma(msg)` only |
| Backend and frontend/backend interface | Supported | Laser-D retains these implementation components unchanged. This does not by itself settle individual source-language features. | `AGENTS.md` project constraint | None | Not a language conformance test |
| Target operating systems | Supported | Windows, Linux, and macOS are required targets. | `AGENTS.md`, `spec-markdown/portability.md` | `spec/portability.dd` | Cross-platform CI needed |
| x86-64 | Supported | x86-64 is the initial supported architecture. | `AGENTS.md`, `spec-markdown/portability.md` | `spec/portability.dd` | Cross-platform CI needed |
| ARM64 | Undecided | Planned when upstream support is suitable; not yet a current language/platform guarantee. | `AGENTS.md` | None | None |
| 32-bit targets | Rejected | Laser-D targets x86-64 only. `-m32` and the deprecated `-m32mscoff` alias are rejected before compilation on every host. | `spec-markdown/portability.md` | `spec/portability.dd` | `m32_option_rejected.d`, `m32mscoff_option_rejected.d` |
...
| Modules (`module.dd`) | Restricted | Core module declarations, namespaces, imports, re-exports, cycles, separate compilation, package modules, and package visibility are supported. `ModuleInfo` and all module lifecycle constructors and destructors are rejected. |
...
| Function contracts (`contracts.dd`) | Rejected | Expression and block `in` preconditions, `out` postconditions, named results, and contract-style `do` bodies are rejected. Validation uses visible ordinary control flow, explicit result handling, or an explicitly written mandatory runtime assertion. |
| Conditional compilation (`version.dd`) | Restricted | `D_BetterC` is always defined; review remaining predefined versions and debug behavior. The absent `D_ModuleInfo`, `D_Exceptions`, and `D_TypeInfo` identifiers follow their settled runtime-feature decisions, as tested by `betterc_default.d` and `upstream_compilable_betterc.d`. |
...
| Memory safety (`memory-safe-d.dd`) | Rejected | The checked `@safe` subset and `@trusted` boundary are not part of Laser-D. Every function is implicitly `@system`, with no safety inference or explicit safety annotations; bounds, ownership, foreign-call validity, and lifetime outside the separately documented templated receiver inference remain programmer responsibilities. |
...
| Private declaration visibility | Supported | `private` declarations at module or aggregate scope are accessible throughout their defining module and inaccessible from other modules. This applies to types, values, functions, templates, fields, methods, and parameterized constructors, using either declaration, block, or colon forms. `private` is not valid on local variables. A private parameterized constructor restricts that constructor but does not suppress struct default initialization or `.init`; `@disable this()` remains rejected separately. | `private_declarations_accepted.d`, `private_module_declaration_rejected.d`, `private_member_rejected.d`, `private_method_rejected.d`, `private_constructor_rejected.d`, `private_local_declaration_rejected.d` |
| General `public` visibility | Undecided | Review explicit `public` on declarations outside the settled public-import behavior. Ordinary declarations retain their existing default visibility while this spelling remains under review. | None |
...
| Named and based enums | Supported | Named enums with primitive or enum base types, explicit values, ordinary auto-increment, and `.init`, `.min`, `.max`, and `.sizeof` properties are supported. Members retain the named enum type through `typeof` and same-type construction; equality remains available through a `const` view. | `aggregate_types_accepted.d`, `upstream_compilable_b16346.d`, `upstream_compilable_b6227.d` |
...
| Package modules | Implementation defect | Package modules are intended to be supported: a `package.d` file declares the fully qualified name of its containing package and may act as an importable facade, including through public imports. Root and nested facades work, but the frontend currently accepts a mismatched declared module name instead of rejecting it. | `package_modules_accepted.d`; mismatched-name rejection coverage needed with the fix |
| Package visibility | Supported | `package` declarations are visible throughout their declaration's package and descendant packages. `package(name)` binds visibility to the named current or ancestor package. Neither form exposes a declaration to unrelated packages, `private` remains module-only, and a non-ancestor boundary is rejected. | `package_visibility_accepted.d`, `package_visibility_rejected.d`, `package_visibility_sibling_rejected.d`, `package_visibility_private_rejected.d`, `package_visibility_nonancestor_rejected.d` |
...
| Runtime assertions | Supported | Runtime `assert`, message forms, `assert(0)`, and assertions inside CTFE-capable functions are supported. Assertions are always emitted and call the platform C runtime assertion-failure entry point directly; release, check, and check-action options cannot remove or redirect them. | `DESIGN.md`, `spec-markdown/expression.md`, `spec/betterc.dd` | None | `runtime_assertions_accepted.d`, `runtime_assert_release_mandatory.d` |
...
| Template mixins | Supported | Parameterized and constrained mixin-template declarations and named or unnamed instances may inject supported declarations at module, struct, union, template, and function-local scope. Injected declarations remain subject to every Laser-D restriction. | `template_mixins_accepted.d`, `mixin_template_restrictions_rejected.d` |
...
| Fixed-size array dimensions | Supported | Fixed-array dimensions are integral compile-time expressions, including large dimensions subject to target representation and compiler resources. Ambiguous `T[name]` syntax is resolved semantically: a value names a dimension, while a type still denotes a rejected associative array. | `static_arrays_and_slices_accepted.d`, `upstream_compilable_fix21684.d`, `associative_arrays_rejected.d` |
| Zero-length fixed-array declarations and scalar initialization | Supported | A fixed array dimension may be zero, including for a struct field. A compatible scalar initializer is accepted and initializes no elements. | `upstream_compilable_test21039.d` |
| Zero-length fixed-array layout and pointers | Undecided | Review size, alignment, containing-field layout, address and pointer behavior, slicing, and runtime use independently of the supported declaration and scalar-initialization forms. | None |
...
| Conventional `string`, `wstring`, and `dstring` aliases | Supported | The minimal implicit `object.d` guarantees aliases for `immutable(char)[]`, `immutable(wchar)[]`, and `immutable(dchar)[]`, respectively; the aliases require neither allocation nor runtime support. | `string_aliases_accepted.d` |
...
| GC-backed array operations | Rejected | Dynamic array literals, concatenation, append, `.dup`, `.idup`, `.capacity`, assignment to dynamic-array `.length`, and array expressions requiring an implicit result allocation are rejected. Array allocation with `new T[n]` is covered by the rejection of every `new` expression. The implicit `object` module omits D's GC-backed `reserve` function; user-defined non-GC functions with that ordinary name remain available through UFCS. Destination-backed vectorized array expressions are tracked separately as Undecided. | `dynamic_array_literal_rejected.d`, `new_array_rejected.d`, `array_concatenation_rejected.d`, `array_gc_properties_rejected.d`, `array_implicit_result_rejected.d` |
...
| Feature | Status | Decision | Tests |
| --- | --- | --- | --- |
| Core templates | Supported | Type, value, alias, variadic, recursive, eponymous, function, aggregate, enum, variable, and alias templates are supported and compose with the reviewed Laser-D feature set. Pattern matching an instantiated template may deduce an alias parameter. | `templates_accepted.d`, `template_supported_features_accepted.d`, `upstream_compilable_isexpalias.d` |
...
| Existing language restrictions | Restricted | Templates, template mixins, and CTFE do not bypass rejected Laser-D features. A construct rejected while parsing cannot reach a template body, and a construct rejected during semantic analysis stays rejected when it arrives through a template instantiation or a mixin-template injection. | `template_string_mixin_rejected.d`, `template_restrictions_rejected.d`, `mixin_template_restrictions_rejected.d` |
...
| Function and parameter reflection | Supported | Function kind, virtual index, return ABI, attributes, variadic style, parameter storage classes, and the current function's parameter tuple may be inspected at compile time. On supported x86-64 targets, `isReturnOnStack` distinguishes a directly returned `int` from an indirectly returned forty-byte fixed-array struct. Class-only results have no valid Laser-D class operands. | `traits_type_and_function_accepted.d`, `upstream_compilable_isreturnonstack.d` |
| Symbol reflection | Supported | Names, membership, members, overloads, parents, protection, visibility, linkage, source location, C++ namespaces, target information, and symbol identity may be inspected. `getOverloads` excludes template overloads by default and includes them when its optional argument is `true`. `getAttributes` remains available but source declarations cannot carry UDAs, so it returns an empty sequence for them. | `traits_symbol_accepted.d`, `bitfields_accepted.d`, `upstream_compilable_b20833.d` |
| Semantic probes | Supported | `__traits(compiles)` and `__traits(isSame)` are supported for compile-time feature detection and identity tests. Function-literal identity ignores parameter names while distinguishing different expression structures. | `traits_symbol_accepted.d`, `upstream_compilable_b19829.d` |
...
2. The normative `spec-markdown/` section containing the Laser-D wording.
...
6. The non-normative upstream `spec/` review source, where applicable.
7. Any difference from upstream D or upstream `-betterC` behavior.
8. Any platform-specific result for Windows, Linux, or macOS.

## Maintenance rule

Update this inventory in the same change that identifies or changes a language
feature. Once a decision is made, update the applicable normative
`spec-markdown/` chapter, `DESIGN.md`, and tests together, and retain the
reviewed `.dd` path in the upstream-source field. Do not silently infer a
language decision from the behavior of the current implementation.
