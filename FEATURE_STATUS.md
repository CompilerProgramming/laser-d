...
| Explicit allocation library | Supported | `laserd.rpmalloc` exposes the reviewed rpmalloc general-allocation and caller-owned first-class heap APIs through implementation-neutral names. `laserd.memory.Arena` selects either zero-initializing rpmalloc defaults or an asserted all-or-none custom callback table. Allocation, ownership, heap thread confinement, and release remain explicit; these modules do not introduce language allocation or garbage collection. | `DESIGN.md`, `library/README.md` | None | `library/test/rpmalloc.d`, `library/test/memory.d` |
